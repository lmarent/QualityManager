	...
	try
	{
        ...
        // going into main loop
         if (qualitymanagerPtr != NULL){
			...
			while ( (evt->getType() == ACTIVATE_RULES) || (evt->getType() == ADD_RULES) )
			{
				if (evt->getType() == ADD_RULES){
					// At this point the processing for the add rules event is finished
					// and when we process it the recurring timer is removed and after discarded.
					// Process the active rules event.
					qualitymanagerPtr->handleEvent(evt, NULL);
				}
				qualitymanagerPtr->evnt.get()->reschedNextEvent(evt);
			...
			cout << "next event type:" << evt->getType() << endl;
			...
			while ( evt->getType() == REMOVE_RULES )
			{
				qualitymanagerPtr->evnt.get()->reschedNextEvent(evt);
				evt = qualitymanagerPtr->evnt.get()->getNextEvent();
			}
