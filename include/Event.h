...
typedef enum
{
      ...
      REMOVE_RULES, //1
      ACTIVATE_RULES, //2
      ADD_RULES_QOS_PROCESSOR, //3
      RESP_ADD_RULES_QOS_PROCESSOR, //4
      CHECK_RULES_QOS_PROCESSOR, //5
      RESP_CHECK_RULES_QOS_PROCESSOR, //6
      DEL_RULES_QOS_PROCESSOR, //7
      RESP_DEL_RULES_QOS_PROCESSOR, //8
      GET_INFO, //9
      GET_MODINFO, //10
      TEST, //11
      REMOVE_RULES_CTRLCOMM, //12
      ADD_RULES_CTRLCOMM, //13
      PROC_MODULE_TIMER, //14
      CTRLCOMM_TIMER, //15
} event_t;
...
public:
    ...
    ProcTimerEvent( int ruleID, int actID, unsigned int tId, time_t duration_sec, time_t duration_msec, unsigned int flags ) :
      Event( PROC_MODULE_TIMER, duration_sec , duration_msec, (flags & TM_RECURRING ? duration_msec : 0), flags & TM_ALIGNED),
      rid(ruleID), actid(actID), tmID(tId)
      {}
