...
/*! \file nqrsh.cpp
    ...
    nqrsh is the NetQoS remote shell
    
    this tool can be used for remote control of NetQoS managers
    it supports all command which are part of the control interface
    

    $Id: nqrsh.cpp 2015-04-01 15:00:00 amarentes $
...
int main( int argc, char *argv[] )
{
  ...
  args.add('s', "NetQoSServer", "<server>", "select netqos server"
           , "MAIN", "server");
  args.add('p', "NetQoSPort", "<port>", "select netQoS port", "MAIN", "port");
  ...
  for(;;) {
      ...
      if (!input.empty()) {
		  ...
		  std::cout << "data to send:" << curl << std::endl;
